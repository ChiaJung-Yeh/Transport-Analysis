...
# **R 語言基礎語法**
本章節系統性介紹 R 語言的重要基礎語法，包含 R 語言中的資料型態（物件）、文字處理方式、控制流程，以及資料讀取語匯出方法，請務必熟悉各物件函式，以便後續程式碼撰寫。  
...
df=lapply(dir_files, read.csv)
```

[`dir()`](#資料來源)函式係讀取指定資料夾內所有檔案名稱；[`read.csv()`](#csv-與-txt-檔)函式用以讀取文字檔案。
