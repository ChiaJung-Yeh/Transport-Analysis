...
str_fun=data.frame(Package=c(rep("`base`", 10), rep("`stringr`", 15)), fun=c("`paste()`","`paste0()`","`toupper()`","`tolower()`","`substr()`","`strsplit()`","`gsub()`","`grep()`","`grepl()`","`regexpr()`","`str_to_lower()`","`str_to_upper()`","`str_to_title()`","`str_starts()`","`str_which()`","`str_locate()`","`str_count()`","`str_sub()`","`str_length()`","`str_pad()`","`str_split()`","`str_flatten()`","`str_glue()`","`str_order()`","`str_sort()`"), description=c("連接字串","連接字串（無分隔）","轉換為大寫","轉換為小寫","擷取字元","拆解字串","取代字元","回傳符合條件的索引值","回傳是否含有特定字元","回傳特定字元出現的位置索引","轉換為大寫","轉換為小寫","首字大寫","回傳是否以特定字元開頭","回傳是否含有特定字元","回傳特定字元出現的位置索引","回傳特定字元出現的次數","取代字元","字串長度","統一字串長度的格式","拆解字串","連接字串","連接字串（含變數）","回傳字串排序索引值","回傳字串排序後結果"))
...
* `sep=`設定分隔符號，例如常見的 .csv 檔即為「逗號」分隔。此引數預設為「,」，若非逗點分隔，需設定此參數。  
