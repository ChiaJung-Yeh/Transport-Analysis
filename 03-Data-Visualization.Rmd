...
### 棒棒糖圖
棒棒糖圖（Lollipop Plot）的呈現與長條圖非常類似，惟長條是利用柱狀表示統計量，而棒棒糖圖則是利用線段表示之，並利用點表示總量，故兩圖的功能甚為類似。此外，在繪製圖表時，縱使圓餅圖是最廣為應用的圖表設計，然而在實務上圓餅圖對於讀者而言無法清楚辨明大小關係（需判斷角度或面積大小，但對人類而言極難判別），致使視覺化效果不彰。因此在類別數量較多或各類別比例相近時，建議使用棒棒圖尚能提升視覺化程度。
...
棒棒糖圖的功能可與長條圖類似，用以呈現數值大小。以`mtcars`資料為例，繪製橫向的長條圖，故設定`x=`引數為 0，而`xend=`引數設定為數值大小（如以下範例中 mpg 變數），`y=`與`yend=`引數則設定為類別變數（如以下範例為 Car 變數）。程式碼撰寫如下。
...
textfig=data.frame(pack=c(rep("`ggplot2`", 2), rep("`ggrepel`", 2), rep("`ggtext`", 2), rep("`ggimage`", 1)),
                   func=c("[`geom_text()`](#標註文字與標籤)","[`geom_label()`](#標註文字與標籤)","[`geom_text_repel()`](#文字避免重疊)`","[`geom_label_repel()`](#文字避免重疊)","[`geom_richtext()`](#html-文字ggtext套件)","[`geom_textbox()`](#html-文字ggtext套件)","[`geom_image()`](#標註插圖)"),
...
* <u>**標註 html 文字與標籤**</u>：[`geom_richtext()`](#html-文字ggtext套件)、[`geom_textbox()`](#html-文字ggtext套件)  
...
### HTML 文字（`ggtext`套件）
`geom_text()`與`geom_label()`函式皆存在其限制，而`ggtext`套件中的`geom_richtext()`與`geom_textbox()`兩函式可更彈性地提供透過 HTML 語法設定文字樣式，此外亦可利用`<span> style=</span>`之 HTML 標籤增加文字樣式的豐富度。常用的 HTML 文字樣式設定彙整如。
