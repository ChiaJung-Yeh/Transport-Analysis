...
除以上調整外，在`ggtern`套件中另外提供諸多函式可調整圖表的細節，如`tern_limits()`可用以設定座標軸的標記間隔，其中設定`breaks=`引數可調整之。以下程式碼中`breaks=seq(0.1,1,by=0.1)`表示由 0.1 始標記，並每 0.1 為間隔標誌 1。另外該套件亦提供許多[主題設定的函式](https://rdrr.io/cran/ggtern/man/ggtern_themes.html)，皆以`theme_`開頭，如以下程式碼中利用`theme_arrownormal()`函式在三角形的邊上加上箭頭，用以表示特定類別的傾向，有利於觀察與判讀。
