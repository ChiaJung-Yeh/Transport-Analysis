...
kable(gv_code, booktabs=T, escape=F, caption="`geom_boxplot()`函式引數設定", col.names=c("引數","功能設定","說明"))%>%
  kable_styling(bootstrap_options=c("striped", "hover"), font_size=14)%>%
  column_spec(1:3 ,extra_css="vertical-align:middle;")%>%
  row_spec(0, bold=T, color="white", background="#8E8E8E")
```
...
分位數在統計上亦能夠顯示資料分布的情形，最常見的如四分位數（`quantile=4`，預設）、十分位數（`quantile=10`），而其落處於正中間的線段即為中位數。若欲建立具有分位數的山脊圖，可利用`stat_density_ridges()`函式，並透過`quantile_lines=T`設定回傳分位數，而`quantiles=`引數則可進一步設定分位數。
...
ggplot(mtcars_scale, aes(variable, Cars, fill=value))+
  geom_tile()+
  scale_fill_distiller(palette="YlOrRd", direction=1)
```
