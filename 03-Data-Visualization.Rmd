...
### 熱力圖
熱力圖（Heatmap）有兩種主要功能，其一是表達資料數值的相對大小，另一則是表達變數間的關係，並透過顏色深淺表達數值大小，以視覺化呈現資料的分布情形。在`ggplot2`套件中可利用`geom_tile()`函式建立熱力圖，該函式撰寫架構如下。
...
除了可以利用`heatmap()`中的樹狀圖表達變數之間的相關性外，亦可先行計算相關係數，而後透過`corrplot`套件中的`corrplot()`函式繪製相關係數熱力圖。請注意未使用此一函式，請務必先行下載`corrplot`套件。`corrplot()`函式的撰寫架構與主要引數設定如下。
...
kable(temp, booktabs=T, escape=F, caption="`geom_tile()`函式引數設定", col.names=c("函式","功能","引數"))%>%
  kable_styling(bootstrap_options=c("striped", "hover"), font_size=14)%>%
...
```{r geom-treemap-eg1, echo=T, eval=T, out.width="80%", fig.align="center", warning=F}
# 人口數統計與區域劃分
...
```{r geom-treemap-eg2, echo=T, eval=F, out.width="80%", fig.align="center", warning=F}
...
```{r geom-treemap-eg2-fig, echo=F, eval=T, out.width="80%", fig.align="center", warning=F}
ggplot(tw_pop, aes(area=people_total, fill=District, label=COUNTYNAME, subgroup=District))+
  geom_treemap()+
...
```{r geom-treemap-eg3, echo=T, eval=F, out.width="80%", fig.align="center", warning=F}
ggplot(tw_pop, aes(area=people_total, fill=District, label=COUNTYNAME, subgroup=District))+
  geom_treemap()+
  geom_treemap_text(size=15, color="#3C3C3C", alpha=0.5, fontface="bold")+
...
```{r geom-treemap-eg3-fig, echo=F, eval=T, out.width="80%", fig.align="center", warning=F}
ggplot(tw_pop, aes(area=people_total, fill=District, label=COUNTYNAME, subgroup=District))+
  geom_treemap()+
...
```{r geom-treemap-eg4, echo=T, eval=F, out.width="80%", fig.align="center", warning=F}
...
```{r geom-treemap-eg4-fig, echo=F, eval=T, out.width="80%", fig.align="center", warning=F}
ggplot(tw_pop, aes(area=people_total, fill=District, label=COUNTYNAME, subgroup=District, subgroup2=Type))+
  geom_treemap(show.legend=F)+
  geom_treemap_text(size=15, color="#3C3C3C", alpha=0.5, fontface="bold", family="A")+
  geom_treemap_subgroup_text(grow=T, color="white", alpha=0.3, fontface="bold", place="center", family="A")+
