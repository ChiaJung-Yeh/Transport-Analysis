...
有時候資料型態並不見得如同前面的範例，將所有節點逐欄羅列並給予統計量，此時我們必須預先處理資料尚能繪製桑基圖。以 2023 年臺灣直轄市間的<H2 id="scmig">社會遷入與遷出資料（scmig）</H2>  為例，其資料記錄結構如下。
...
三元圖可以表達不同的三個類別之間的比例關係，以<H2 id="tw-vote">臺灣 2024 年總統選舉資料（tw_vote）</H2>為例，有三個政黨推派候選人，故相當適合透過三元圖表達投票結果。以下程式碼中先行讀取全臺各鄉鎮的總統投票結果資料（tw_vote），其中記錄總票數（Votes_Total），以及各政黨的得票率，其中包含 KMT（國民黨）、DPP（民進黨）、TPP（民眾黨）三者。
...
最後繪製圖表如下，其中利用`geom_curve()`函式標記臺中市到達其他直轄市的社會遷出，並透過線條粗細表示移動人口的量。此外透[`geom_point()`](#散布圖)函式標記各直轄市位置，且利用[`geom_text()`](#標註文字)函式標註直轄市名稱。
...
textfig=data.frame(pack=c(rep("`ggplot2`", 2), rep("`ggrepel`", 2), rep("`ggtext`", 2), rep("`ggimage`", 1)),
                   func=c("[`geom_text()`](#標註文字)","[`geom_label()`](#標註標籤)","[`geom_text_repel()](#文字避免重複)`","[`geom_label_repel()`](#文字避免重複)","[`geom_richtext()`](#html-文字ggtext套件)","[`geom_textbox()`](#html-文字ggtext套件)","[`geom_image()`](#標註插圖)"),
...
gt_code=data.frame(argument=c("`x=`","`y=`","`label=`","`color=`","`size=`","`alpha=`","`angle=`","`family=`","`fontface=`","`hjust=`","`vjust=`","`nudge_x=`","`nudge_y=`"),
                   setting=c('橫軸座標（必須<A href="#ggplot-aes">`aes()`</A>函式內）','縱軸座標（必須<A href="#ggplot-aes">`aes()`</A>函式內）','標註文字（必須<A href="#ggplot-aes">`aes()`</A>函式內）',"文字顏色","文字大小","文字透明度","文字角度","文字字體","文字字型","文字橫軸對齊","文字縱軸對齊","文字橫軸位移","文字縱軸位移"), explain=c("橫軸座標欄位","縱軸座標欄位","標註文字欄位","請參考<A href='#geom-setting-color'>顏色設定</A>","數值","數值，介於 0（完全透明）與 1（完全不透明）間","數值，介於 0° 至 360° 間","請參見[文字字體與字型](#文字字體與字型)","`plain`:一般（預設）<br>`bold`:粗體<br>`italic`:斜體<br>`bold.italic`:粗斜體","**數值：**`0`:靠左；`0.5`:置中；`1`:靠右<br>**文字：**`left`:靠左；`middle`:置中；`right`:靠右","**數值：**`0`:靠下；`0.5`:置中；`1`:靠上<br>**文字：**`bottom`:靠下；`center`:置中；`top`:靠上","將文字橫向移動單位數","將文字縱向移動單位數"))
...
以下範例展示設定字體與字型的方法，再次以<A href='#tw-vote'>2024 年總統大選</A>資料為例，並萃取新竹縣的資料，繪製各鄉鎮的政黨得票率比例長條圖，最後在長條上標註得票比例及行政區名稱，程式碼撰寫如下。

```{r font-family-type-eg1, echo=T, eval=F, warning=F}
# 先在系統中設定字體
...
ggplot()+
  ...
  geom_text(data=hs_vote, aes(x=TOWNNAME, y=lab_y, label=paste0(round(Rate*100, 1), "%")), size=3, family="TNR", nudge_y=-0.05)+
  geom_text(data=hs_vote, aes(x=TOWNNAME, y=1, label=TOWNNAME), size=3, family="biaukai", angle=45, hjust=1)
```

```{r font-family-type-eg2, echo=F, eval=T, out.width="80%", fig.align="center", warning=F}
hs_vote=read.csv("https://raw.githubusercontent.com/ChiaJung-Yeh/Transport-Analysis/master/Data/tw_president_vote.csv")%>%
  filter(COUNTYNAME=="新竹縣")%>%
  melt(id.vars="TOWNNAME", measure.vars=c("TPP","DPP","KMT"), variable.name="Party", value.name="Rate")
hs_vote$Party=factor(hs_vote$Party, levels=c("TPP","DPP","KMT"))
...
ggplot()+
  geom_col(data=hs_vote, aes(x=TOWNNAME, y=Rate, fill=Party), width=0.5)+
  geom_text(data=hs_vote, aes(x=TOWNNAME, y=lab_y, label=paste0(round(Rate*100, 1), "%")), size=3, family="E", nudge_y=-0.05)+
...
* [`family=`](#文字字體與字型):設定文字的字體  
* [`angle=`](#文字角度與對齊):設定文字角度  
* [`hjust=`](#文字角度與對齊):設定文字對齊  
* [`nudge_y=`](#文字角度與對齊):設定文字位移  

**各引數的設定請參見\@ref(#文字字體與字型)至\@ref(#文字位移)章節！**
...
### 文字字體與字型
<p style="color:#003D79">請注意文字字體與字型設定不限於`geom_text()`函式，亦包含可擴充 html 文字的[`geom_richtext()`與`geom_textbox()`](#html-文字ggtext套件)，以及用以設定主題的[`element_text()`與`element_markdown()`](#細節樣式設定)等，任何有關文字設定的套件通常皆可透過本小節所述方式設定字體與字型。</p>
...
### 文字角度與對齊
<p style="color:#003D79">請注意文字角度與對齊設定不限於`geom_text()`函式，亦包含可擴充 html 文字的[`geom_richtext()`與`geom_textbox()`](#html-文字ggtext套件)，以及用以設定主題的[`element_text()`與`element_markdown()`](#細節樣式設定)等，任何有關文字設定的套件通常皆可透過本小節所述方式設定角度與對齊。</p>

所有有關文字角度與對齊的函數，其引數設定方式皆同。調整角度利用`angle=`引數；調整對齊則利用`hjust=`與`vjust`引數，各引數設定參見表表\@ref(tab:font-angle-just-tab)，設定結果參照圖\@ref(fig:font-angle-just)所示。
...
### 文字位移
<p style="color:#003D79">請注意文字位移設定不限於`geom_text()`函式，亦包含可擴充 html 文字的[`geom_richtext()`與`geom_textbox()`](#html-文字ggtext套件)，以及用以設定主題的[`element_text()`與`element_markdown()`](#細節樣式設定)等，任何有關文字設定的套件通常皆可透過本小節所述方式設定字體與字型。</p>

文字位移是指將所有文字或標籤往特定方向移動，如此可統一修正文字的位置，齊
