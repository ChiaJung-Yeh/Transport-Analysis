...
kable(temp, booktabs=T, escape=F, caption="`treemapify`重要函式", col.names=c("函式","功能","引數"))%>%
  ...
  collapse_rows(3)%>%
  row_spec(0, bold=T, color="white", background="#8E8E8E")%>%
  footnote("`geom_treemap_subgroup`後面可接數字，表示各子類別的層級，包含：<br>`geom_treemap_subgroup2_XX`與`geom_treemap_subgroup3_XX`")
```
...
```{r geom-sankey-fig2, echo=F, eval=T, out.width="60%", fig.align="center", fig.cap="桑基圖架構示意圖"}
include_graphics("./figure/sankey_diagram_table.png")
```

在繪製桑基圖時，可以透過`ggplot()`函式設定各階層與流動量，引數設定如表\@ref(tab:geom-sankey-code1-tab)所示。

```{r geom-sankey-code1-tab, echo=F, eval=T}
gs_code=data.frame(argument=c("`axis[1-9]=`","`y=`"), setting=c("設定階層","設定流動量"), explain=c("`axis1=欄位名稱`,`axis2=欄位名稱`,...","流動量欄位"))
...
除上述的階層與流動量設定外，`geom_alluvium()`函式可用以設定節線的樣式；`geom_stratum()`函式則用以設定節點樣式，函式中的引數設定與其他圖表雷同，而通常會設定`fill=`引數依據不同類別填色。
...
有時候資料型態並不見得如同前面的範例，將所有節點逐欄羅列並給予統計量，此時我們必須預先處理資料尚能繪製桑基圖。以 2023 年臺灣直轄市間的社會遷入與遷出資料（scmig）為例，其資料記錄結構如下。
